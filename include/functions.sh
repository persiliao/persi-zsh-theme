...
persi_get_repo_name() {
  PERSI_GIT_REPO_NAME=$(git remote -v | grep -i origin | head -1 | awk '{print $2}' | sed 's#^https://\([^/]*\)/\([^\.]*\)#\2#g')
  ...
  export PERSI_GIT_REPO_NAME
  export PERSI_GIT_REPO_NAME_GIT=${PERSI_GIT_REPO_NAME%.git}
  return 0
}
