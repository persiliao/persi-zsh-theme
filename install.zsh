...
setup_install_recommended_plugin() {
  PERSI_ZSH_RECOMMENDED_PLUGINS=(wd git gitignore git-flow git-flow-avh gh docker docker-compose kubectl npm node golang wp-cli composer yarn systemd systemadmin mvn nvm pip redis-cli supervisor gradle flutter brew rust jenv)
