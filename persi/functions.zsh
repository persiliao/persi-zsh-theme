...
persi_install_plugin(){
    ...
    if [ $PERSI_INSTALL_PLUGIN = 'y' ]; then
        PERSI_ZSH_RECOMMENDED_PLUGIN=(zsh_reload wd git gitignore git-flow git-flow-avh docker docker-compose kubectl npm node golang wp-cli composer yarn systemd systemadmin mvn nvm pip redis-cli supervisor gradle)
